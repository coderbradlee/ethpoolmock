...
func NewProxy2(cfg *ProxyConf, backend *storage.RedisClient) *ProxyServer {
	if len(cfg.Name) == 0 {
		log.Fatal("You must set instance name")
	}
	policy := policy.Start(&cfg.Policy, backend)

	proxy := &ProxyServer{config: cfg, backend: backend, policy: policy}
	proxy.diff = util.GetTargetHex(cfg.Difficulty)
	...
	proxy.hashrateExpiration = util.MustParseDuration(cfg.HashrateExpiration)
	...
	stateUpdateIntv := util.MustParseDuration(cfg.StateUpdateInterval)
...
// 	proxy := &ProxyServer2{config: cfg, backend: backend, policy: policy}
...
func (s *ProxyServer) isSick() bool {
	...
	if s.config.HealthCheck && x >= s.config.MaxFails {
		return true
	}
	return false
}
